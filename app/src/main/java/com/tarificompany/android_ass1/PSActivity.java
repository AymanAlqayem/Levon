...
public class PSActivity extends AppCompatActivity {
    ...
    public void loadItems() {
        ...
        title.setText("Playstation games");

        // Load items from ItemManager and filter for Playstation games.
        ArrayList<Item> itemArrayList = new ArrayList<>();
        for (Item item : ItemManager.getAllItems(this)) {
            if (item.getId() >= 186 && item.getId() <= 193) {
        ...
        itemsList.setOnItemClickListener((parent, view, position, id) -> {
            ...
            Intent intent = new Intent(PSActivity.this, ItemDetailActivity.class);
